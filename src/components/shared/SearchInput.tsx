  ...
  (
    ...
    return (
      <div className={`relative w-full ${maxWidthClasses[maxWidth]} ${containerClassName}`}>
        <input
          ref={ref}
          className={`w-full rounded-xl border border-slate-400 bg-white ${sizeClasses[size]} ${
            shouldShowIcon ? 'pr-12' : ''
          } transition placeholder:text-slate-400 focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-500/20 disabled:cursor-not-allowed disabled:opacity-60 dark:border-slate-500 dark:bg-slate-800 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:border-sky-400 ${className}`}
          {...inputProps}
        />
        {shouldShowIcon && (
          <div
