...
export default function SimpleMode({
  ...
  const serviceExample = isAzure ? 'Compute, Storage, Network' : 'directory, graph, users';
    ...
    <>
      <div className="space-y-2 relative max-w-xl" ref={serviceDropdownRef}>
        ...
        <SearchInput
          ...
          placeholder={isLoadingServices ? "Loading..." : `Search for a service (e.g., ${serviceExample})`}
        ...
        />
          ...
          {showServiceDropdown && serviceSearch && filteredServices.length === 0 && (
            <div className="absolute z-10 mt-1 w-full rounded-lg border border-slate-200 bg-white p-4 text-center text-sm text-slate-600 shadow-lg dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400">
              No {config.labels.serviceLabel.toLowerCase()} match &ldquo;{serviceSearch}&rdquo;
            </div>
          )}
      </div>
      ...
      {selectedService && (
        <div className="space-y-2">
          ...
          <SearchInput
            ...
            placeholder={`Filter ${config.labels.actionLabel.toLowerCase()} (e.g., read, write, delete)`}
            maxWidth="full"
            icon={null}
          />
          ...
          {isLoadingActions ? (
            ...
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-center text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400">
              {actionSearch ? `No ${config.labels.actionLabel.toLowerCase()} match your filter` : `No ${config.labels.actionLabel.toLowerCase()} available for this ${config.labels.serviceLabel.toLowerCase()}`}
            </div>
          )}

          {isAzure && availableActions.length > 0 && (
            <div className="mt-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800 dark:border-amber-400/30 dark:bg-amber-500/10 dark:text-amber-300">
              <strong>Note:</strong> Simple mode only shows actions explicitly defined in roles. Some actions (like bastionHosts) may be covered by wildcards (e.g., <code className="rounded bg-amber-100 px-1 py-0.5 font-mono dark:bg-amber-900/40">{config.placeholders.wildcardExample}</code>) and won&apos;t appear in this list. Use <strong>Advanced mode</strong> to search for any action.
            </div>
          )}
        </div>
      )}
      ...
      <SelectionChips
        heading={`Selected ${config.labels.actionLabel}`}
