...
/** One row of /data/actions-index.json, generated by lib/rbacCacheGenerator. */
...
function getMatchSpecificity(requiredAction: string, grantedPattern: string): MatchSpecificity | null {
  if (!matchesWildcard(grantedPattern, requiredAction)) {
    return null;
  }
...
function findBestGrantedMatch(
  ...
  for (const allowed of allowPatterns) {
    ...
    const isDenied = denyPatterns.some((denied) => matchesWildcard(denied, requiredPermission));
    if (isDenied) {
      continue;
    }
